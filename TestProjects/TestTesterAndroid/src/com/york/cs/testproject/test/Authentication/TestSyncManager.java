...
import com.york.cs.services.SyncAdapter.SyncManager;
...
public class TestSyncManager extends ApplicationTestCase<Application> {
	...
	public void testSyncAPINow(){
	
		SyncManager.syncCurretUserNow(getContext());
		
		
	}
	
	
	public void testSyncAPIPeriod(){
		
		
		
		SyncManager.syncScheduledPeriodically(context,(long) (60*60*2));
		

		SyncManager.isSyncActive(getContext());
		
		SyncManager.isSyncPending(getContext());
	}
	
public void testSyncAPIPeriodNet(){
...
QueryEnumerator result;
		...
		// QueryEnumerator result;
		/*
		 * try { Log.d("updating owner with new user", "Total "); result =
		 * query.run();
		 * 
		 * Log.d("updateing channel with new user", "Total Docs: " +
		 * result.getCount());
		 * 
		 * for (Iterator<QueryRow> it = result; it.hasNext();) { QueryRow row =
		 * it.next();
		 * 
		 * String oldOwner = (String) row.getDocument().getProperty(
		 * "owner_id"); System.out.println("System.out.println(oldOwner)" +
		 * oldOwner); System.out.println("email: " + email);
		 * 
		 * if (oldOwner == null) { userNull = userNull + 1; } else if (email !=
		 * null && email.equals(oldOwner)) { userNotNull = userNotNull + 1;
		 * continue; } else { userDifferent = userDifferent + 1;
		 * 
		 * }
		 * 
		 * }
	...
	private Query getQueryByOwner(Database database) {
		...
		//com.couchbase.lite.View view0 = database.getView("tester"
			//	+ "byOwnerId");
		//view0.delete();
		com.couchbase.lite.View view = database
				.getView("tester" + "byOwnerId");
		if (view.getMap() == null) {
			Mapper map = new Mapper() {
				@Override
				public void map(Map<String, Object> document, Emitter emitter) {
					if(document.get("owner_id")!=null)
					emitter.emit(document.get("owner_id"), document.get("owner_id"));
					else emitter.emit("null", document.get("owner_id")); 
